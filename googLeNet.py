...
def inception_model(input, filters_1x1, filters_3x3_reduce, filters_3x3, filters_5x5_reduce, filters_5x5, filters_pool_proj):
	...
	conv_1x1 = Conv2D(filters=filters_1x1, kernel_size=(1, 1), padding='same', activation='relu', kernel_initializer = 'he_normal')(input)
	
	conv_3x3_reduce = Conv2D(filters=filters_3x3_reduce, kernel_size=(1, 1), padding='same', activation='relu', kernel_initializer = 'he_normal')(input)
	
	conv_3x3 = Conv2D(filters=filters_3x3, kernel_size=(3, 3), padding='same', activation='relu', kernel_initializer = 'he_normal')(conv_3x3_reduce)
	
	conv_5x5_reduce  = Conv2D(filters=filters_5x5_reduce, kernel_size=(1, 1), padding='same', activation='relu', kernel_initializer = 'he_normal')(input)
	
	conv_5x5 = Conv2D(filters=filters_5x5, kernel_size=(5, 5), padding='same', activation='relu', kernel_initializer = 'he_normal')(conv_5x5_reduce)
	
	maxpool = MaxPooling2D(pool_size=(3, 3), strides=(1, 1), padding='same')(input)
	
	maxpool_proj = Conv2D(filters=filters_pool_proj, kernel_size=(1, 1), strides=(1, 1), padding='same', activation='relu', kernel_initializer = 'he_normal')(maxpool)
	
	inception_output = concatenate([conv_1x1, conv_3x3, conv_5x5, maxpool_proj], axis=3)  # use tf as backend
	
	return inception_output

def extra_output(input, pool_size, pool_stride, filter_num, fc1_num, fc2_num):

	averagePool = AveragePooling2D(pool_size=pool_size, strides=pool_stride, padding='same')(input)

	conv = Conv2D(filters=filter_num, kernel_size=(1, 1), strides=(1, 1), padding='same', activation='relu', kernel_initializer = 'he_normal')(averagePool)

	conv = Flatten()(conv)
	
	fc1 = Dense(fc1_num, activation = 'relu')(conv)

	drop = Dropout(rate=0.7)(fc1)

	fc2 = Dense(fc2_num, activation  ='sigmoid')(drop)
...
def define_model(weight_path = None):
    ...
    averagepool1_7x7_s1 = AveragePooling2D(pool_size=(7, 7), strides=(1, 1), padding='same')(inception_5b)
...
def get_googlenet_train():
	'''
    with two extra output
    '''
    inputs = Input(shape = (224,224,3))

    conv1_7x7_s2 = Conv2D(filters=64, kernel_size=(7, 7), strides=(2, 2), padding='same', activation='relu', kernel_initializer = 'he_normal')(inputs)

    maxpool1_3x3_s2 = MaxPooling2D(pool_size=(3, 3), strides=(2, 2), padding='same')(conv1_7x7_s2)

    conv2_3x3_reduce = Conv2D(filters=64, kernel_size=(1, 1), padding='same', activation='relu', kernel_initializer = 'he_normal')(maxpool1_3x3_s2)

    conv2_3x3 = Conv2D(filters=192, kernel_size=(3, 3), padding='same', activation='relu', kernel_initializer = 'he_normal')(conv2_3x3_reduce)
    ...
    out_0 = extra_output(inception_4a, (5,5), 3, 128, 1024, 1)
    ...
    out_1 = extra_output(inception_4d, (5,5), 3, 128, 1024, 1)
    ...
    drop1 = Dropout(rate=0.4)(averagepool1_7x7_s1)
    drop1 = Flatten()(drop1)
    linear = Dense(self.label_num, activation = 'sigmoid')(drop1)
    last = linear


    model = Model(inputs=inputs, outputs=[out_0, out_1, last])
    model.summary()
    model.compile(optimizer = Adam(lr = 1e-4), loss_weights=[1, 0.3, 0.3], loss = 'binary_crossentropy', metrics=['accuracy'])
    return model
